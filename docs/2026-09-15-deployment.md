...
站点不需要登录。访客打开辩论、读 AI 摘要、质询、提问与自动编排，都会消耗部署者的知乎搜索额度和 DeepSeek 费用；首页的 `/api/topics` 还会在后台触发热榜收录。知乎搜索在本地不限次数（账号每日额度 5000 次），只在知乎返回限流时暂停 1 分钟；热榜每小时最多请求一次，额度用完后当天不再请求，计数存在 `/var/lib/zhibian/hot-usage.json`。DeepSeek 调用没有总量上限。需要收紧时，可以在 Caddy 片段里加 `basic_auth`。
